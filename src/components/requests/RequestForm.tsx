...
/**
 * RequestForm — create and edit form for borrower's form requests.
 *
 * Sections:
 *   1. Request Details (org, event, dates, venue, contact)
 *   2. Assets Requested (AssetSelector)
 *   3. Paperwork Status (Secretary/Auditor/President checkboxes)
 *   4. Final Status (dropdown — Auditor sets manually)
 *   5. Remarks
 *   6. Attachments (shown only when editing an existing request)
 *
 * Rules:
 *   - New requests always start as Pending (enforced server-side too).
 *   - Auditor manually changes status — checkboxes do NOT auto-approve.
 *   - Asset snapshot fields are saved alongside asset_id.
 */
...
interface RequestFormProps {
  ...
  /** Attachments for the existing request (fetched server-side) */
...
export default function RequestForm({
  ...
  const [form, setForm] = useState<RequestFormData>(
    existing ? toFormData(existing) : EMPTY_FORM
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <section>
        <h3 className="text-white text-sm font-semibold mb-4 pb-2 border-b border-gray-800">
          Request Details
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.organization_name}
          ...
          <div>
            ...
            <input type="text" value={form.event_name}
              ...
              placeholder="e.g. JS Workshop 2025"
              className={inputClass} disabled={submitting} />
          ...
          <div className="md:col-span-2">
            ...
            <input type="text" value={form.venue}
              ...
              placeholder="e.g. AVR 3rd Floor, Main Building"
              className={inputClass} disabled={submitting} />
          ...
          <div>
            ...
            <input type="text" value={form.contact_person ?? ''}
              ...
              placeholder="Name of RSO contact"
              className={inputClass} disabled={submitting} />
          </div>

          <div>
            <label className={labelClass}>Adamson Email</label>
            <input type="email" value={form.adamson_email ?? ''}
              onChange={(e) => set('adamson_email', e.target.value)}
              placeholder="e.g. orgname@adamson.edu.ph"
              className={inputClass} disabled={submitting} />
          </div>
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">
          Check each box after the physical form has been signed in real life.
          All three are optional — a request can be saved at any stage.
        </p>
            ...
            <label key={key} className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  // Update the checkbox and derive the new status in one step
                  setForm((prev) => {
                    const next = { ...prev, [key]: checked }
                    const derived = deriveStatusFromPaperwork(
                      next.secretary_signed,
                      next.auditor_signed,
                      next.president_approved,
                      prev.status
                    )
                ...
                disabled={submitting}
                className="w-4 h-4 rounded border-gray-600 bg-gray-800 text-blue-600 focus:ring-blue-600 focus:ring-offset-gray-900"
              />
        ...
        {/* Live hint based on current checkbox state */}
        <div className={`mt-3 px-3 py-2 rounded-lg text-xs ${
          form.secretary_signed && form.auditor_signed && form.president_approved
            ? 'bg-green-500/10 border border-green-500/20 text-green-400'
            : 'bg-gray-800/60 border border-gray-700 text-gray-500'
        }`}>
          {form.secretary_signed && form.auditor_signed && form.president_approved
      ...
      <section>
        ...
        <p className="text-gray-500 text-xs mb-3">
          Status is set automatically based on paperwork. You can also override it manually.
        </p>
        <div>
      ...
      <div className="flex justify-end gap-3 pt-2">
        <button
          ...
          onClick={onCancel}
          disabled={submitting}
          className="px-4 py-2 rounded-lg text-sm font-medium text-gray-300 bg-gray-800 hover:bg-gray-700 border border-gray-700 transition-colors disabled:opacity-50"
        >
          Cancel
        </button>
        <button
          type="submit"
          disabled={submitting}
          className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
