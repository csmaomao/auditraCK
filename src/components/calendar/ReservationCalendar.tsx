/**
 * ReservationCalendar — monthly grid calendar built with CSS grid.
 * No external calendar library required.
 *
 * Shows approved requests as event chips on their event_date.
 * Clicking a chip calls onSelectEvent so the parent can open the detail modal.
 */
...
export default function ReservationCalendar({
  ...
  // Build a map of event_date → events for O(1) lookup per cell
        ...
        {Array.from({ length: totalCells }).map((_, idx) => {
          ...
          if (!isCurrentMonth) {
            return (
              <div
                key={idx}
                className="min-h-[56px] sm:min-h-[80px] border-b border-r border-gray-800/50 bg-gray-900/30"
              />
            )
          }
          ...
          return (
            <div
              key={idx}
              className="min-h-[56px] sm:min-h-[80px] border-b border-r border-gray-800/50 p-1 sm:p-1.5"
            >
              {/* Day number */}
                ...
                {dayEvents.slice(0, 2).map((event) => {
                  ...
                  return (
                    <button
                      ...
                      title={`${event.event_name} — ${event.organization_name}`}
                      className={`
                        w-full text-left px-1 sm:px-1.5 py-0.5 rounded text-xs truncate
