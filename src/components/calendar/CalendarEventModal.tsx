...
export default function CalendarEventModal({ event, onClose }: CalendarEventModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-3 sm:p-4 bg-black/60 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="calendar-event-title">
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl my-4 sm:my-8">

        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <div className="min-w-0 flex-1">
            <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-green-500/20 text-green-600 dark:text-green-400 border border-green-500/30 mb-1">Approved</span>
            <h2 id="calendar-event-title" className="text-gray-900 dark:text-white text-base font-semibold truncate">{event.event_name}</h2>
          </div>
          <button type="button" onClick={onClose} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors ml-3 shrink-0" aria-label="Close">
        ...
        <div className="px-4 sm:px-6 py-5 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Time" value={event.start_time || event.end_time ? `${formatTime(event.start_time)} – ${formatTime(event.end_time)}` : undefined} />
          ...
          {event.remarks && (
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300 text-sm whitespace-pre-wrap">{event.remarks}</p>
            </div>
          )}

          <div>
            <p className="text-gray-900 dark:text-white text-sm font-semibold mb-2 pb-2 border-b border-gray-200 dark:border-gray-800">Requested Assets</p>
